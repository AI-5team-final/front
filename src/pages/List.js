...
const List = () => {
    ...
    useEffect(() => {
        const fetchMatches = async () => {
            if (!role && !isTutorialMode) return;

            const file = role === 'APPLICANT' ? resumeFile : jobPostFile;
            if (!file && !isTutorialMode) return;
            ...
            try {
                if (isTutorialMode) {
                    // 튜토리얼 모드일 때는 가상 데이터 사용
                    const mockData = role === 'APPLICANT' ? [
                        {
                            "total_score": 100,
                            "resume_score": 50,
                            "selfintro_score": 50,
        ...
        fetchMatches();
    }, [resumeFile, jobPostFile, role, isTutorialMode]);


    if (!role && !isTutorialMode) return null;
    ...
    return (
        ...
        <main className={`l-list`}>
            {!loading && ready && (role === 'APPLICANT' || isTutorialMode) && <ListApplicant />}
